...
"""
factions = {'first': lambda x: x + '  ' + 'гений', 'second': lambda x: x + ' ' + 'сверхразум', 'third': lambda x: 'Просто' +' ' + x}
...
while x != 'off':
    if x[-1] in genious:
        print(factions['first'](x))
        x = input('Введите имя(off - выключить): ')
    elif x[-1] in opmind:
        print(factions['second'](x))
        x = input('Введите имя(off - выключить): ')
    else:
        print(factions['third'](x))
        x = input('Введите имя(off - выключить): ')
